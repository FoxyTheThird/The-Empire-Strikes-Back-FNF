...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			var offset:Float = 108 - (Math.max(optionShit.length, 6) - 6) * 80;
			...
			menuItem.y += i * -40; // adjust y coordinate based on index i
			menuItem.ID = i;
			// menuItem.screenCenter(X);
			menuItems.add(menuItem);
			// 0.135 is original value
			var scr:Float = (optionShit.length - 6) * 0.27;
			scr = 0;
	...
	override function update(elapsed:Float)
	{
		...
		// How to control the positions of the menu buttons
		menuItems.forEach(function(spr:FlxSprite)
		{
			spr.x = 25;
		});
